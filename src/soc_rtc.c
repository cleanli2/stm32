...
char* get_rtc_time(date_info_t*dit)
{
    slprintf(t_d, "%d.%b.%b %b:%b:%b W%d",
            calendar.w_year,
            hex2bcd(calendar.w_month),
            hex2bcd(calendar.w_date),
            hex2bcd(calendar.hour),
            hex2bcd(calendar.min),
            hex2bcd(calendar.sec),
            hex2bcd(calendar.week)
           );
    if(dit){
...
void adjust_second(int sec)
{
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_PWR | RCC_APB1Periph_BKP, ENABLE);	//使能PWR和BKP外设时钟  
	PWR_BackupAccessCmd(ENABLE);	//使能RTC和后备寄存器访问 
	RTC_SetCounter(timecount);	//设置RTC计数器的值

	RTC_WaitForLastTask();	//等待最近一次对RTC寄存器的写操作完成  	
    timecount=0;
    timecount=RTC_GetCounter();	 
    lprintf("get ct %d after adj\n", timecount);
	RTC_Get();
}
